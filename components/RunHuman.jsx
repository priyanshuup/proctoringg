...
class RunHuman extends Component {
  ...
  render() {
    ...
    if (!this.video.paused) {
      ...
      const { gesture: interpolatedGesture, object: interpolatedObject } = interpolated;
      ...
      const checkGesture = (gesture, storageKey, warningThreshold) => {
        if (interpolatedGesture.some((item) => item.gesture === gesture)) {
          ...
          if (currentCount >= warningThreshold) {
            this.setState((prevState) => ({ warningCount: prevState.warningCount + 1 }));
            console.log(
              `Warning: You are looking ${gesture} too often! (Warning Count: ${this.state.warningCount + 1})`
            );
      ...
      checkGesture("head down", "headDownCount", 30);
      checkGesture("facing left", "headLeftCount", 50);
      checkGesture("facing right", "headRightCount", 50);
      checkGesture("head up", "headUpCount", 50);

      if (interpolatedObject.some((item) => item.label === "person" && item.score > 0.25)) {
        const personCount = interpolatedObject.filter((item) => item.label === "person").length;
        if (personCount >= 2) {
          this.setState((prevState) => ({ warningCount: prevState.warningCount + 1 }));
          console.log("More than 1 person detected in the frame");
          console.log(
            `Warning: More than 1 person detected! (Warning Count: ${this.state.warningCount + 1})`
          );
        }
      } else if (interpolatedObject.some((item) => item.label !== "person")) {
      ...
      if (interpolatedObject.some((item) => item.label === "cell phone")) {
        ...
        if (currentUpCount >= warningThreshold) {
          this.setState((prevState) => ({ warningCount: prevState.warningCount + 1 }));
          console.log(
            `Warning: Cell Phone Detected (Warning Count: ${this.state.warningCount + 1})`
          );
